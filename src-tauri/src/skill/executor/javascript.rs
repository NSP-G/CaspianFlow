...
//!
//! Executes `.js` scripts using `node`.
...
/// Adapter for executing JavaScript skills.
///
/// Uses `node` as the interpreter. The entry file (e.g. `script.js`)
/// is passed as the sole argument, and input JSON is written to stdin.
pub struct JavascriptRuntimeAdapter;

#[async_trait::async_trait]
impl RuntimeAdapter for JavascriptRuntimeAdapter {
    async fn check_available(&self) -> ExecutorResult<()> {
            ...
            .await
        {
            ...
            Ok(_) => Err(ExecutorError::RuntimeNotFound {
                runtime: SkillRuntimeType::Javascript.to_string(),
                reason: "node exited with non-zero status".to_string(),
            }),
            Err(e) => Err(ExecutorError::RuntimeNotFound {
                runtime: SkillRuntimeType::Javascript.to_string(),
                reason: format!("node not found: {e}"),
            }),
        }
    }
...
mod tests {
    ...
    fn make_skill(entry: &str, path: &str) -> Skill {
        Skill {
            ...
            display_name: "Test JS".to_string(),
            version: "1.0.0".to_string(),
            description: "Test".to_string(),
            category: "test".to_string(),
            trigger_phrases: vec![],
            runtime: SkillRuntime {
                runtime_type: SkillRuntimeType::Javascript,
    ...
    fn test_build_args_entry_not_found() {
        let adapter = JavascriptRuntimeAdapter;
        let skill = make_skill("nonexistent.js", "/tmp");
    ...
    fn test_build_args_success() {
        ...
        std::fs::write(&script_path, "console.log('hello')").unwrap();

        let skill = make_skill("script.js", tmp.path().to_str().unwrap());
        let adapter = JavascriptRuntimeAdapter;
        let (exec, args) = adapter.build_args(&skill).unwrap();
        assert_eq!(exec, "node");
        assert_eq!(args.len(), 1);
        assert!(args[0].ends_with("script.js"));
    }
